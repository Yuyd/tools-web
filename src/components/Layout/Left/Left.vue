...
interface LeftAdvItem {
  img: string
  url?: string
}

const parseLeftAdv = (raw: string): LeftAdvItem[] => {
  const value = raw.trim()
  if (!value) return []
  try {
    const parsed = JSON.parse(value)
    const list = Array.isArray(parsed) ? parsed : [parsed]
    return list.filter((item): item is LeftAdvItem => Boolean(item?.img))
  } catch {
    return []
  }
}
...
.ad-container :deep(.el-carousel__item) {
  cursor: pointer;
}
